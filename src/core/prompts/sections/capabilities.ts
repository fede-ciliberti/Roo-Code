...
- You can use the execute_command tool to run commands on the user's computer whenever you feel it can help accomplish your task. When you need to execute a CLI command, you must provide a clear explanation of what the command does. Prefer to execute complex commands over creating shell scripts since they are more flexible and easier to run. Interactive and long-running commands are allowed since they will run in the background and you will be kept updated on their status. You can use a "background": "true" parameter to run commands in the background without blocking the workflow.
- The execute_command tool supports a "background" parameter: set "background": "true" to run commands in the background without blocking the workflow (commands will execute asynchronously), or omit it or set "background": "false" for the default behavior (wait for command completion).${
		supportsComputerUse
